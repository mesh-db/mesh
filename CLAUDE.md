...
- **Not yet implemented:** `apoc.path.*` expansion, `apoc.periodic.*` (needs batch-transaction semantics), `apoc.cypher.run*` (needs runtime Cypher evaluation), `apoc.load.*` / `apoc.export.*` (file I/O and format parsers), `apoc.trigger.*` (DB-level event system).
