...
- **Trigger procedures** — behind `apoc-trigger`: `apoc.trigger.install(databaseName, name, statement, selector, config)`, `apoc.trigger.drop(databaseName, name)`, `apoc.trigger.list()`. After a successful commit, registered triggers run their Cypher body with the tx diff exposed as `$createdNodes` / `$createdRelationships` / `$deletedNodes` / `$deletedRelationships` params. Triggers persist in a dedicated `trigger_meta` RocksDB CF and survive restart. A thread-local suppression guard prevents infinite self-triggering when a trigger's own writes would otherwise fire it again. Install/drop emit `GraphCommand::InstallTrigger` / `DropTrigger` through the writer so the cluster commit path replicates the registry across every peer: Raft-mode rides the existing log + per-peer applier (with a registry-cache refresh hook on `StoreGraphApplier`), and routing-mode rides the existing DDL fan-out via two new `InstallTrigger` / `DropTrigger` gRPC methods on `MeshWrite`. Firing is leader/coordinator-only — exactly one peer per logical commit fires, matching the single-shot semantics callers expect.
- **Deferred — trigger scope gaps.** The shipped surface only covers the `after` phase (`before` / `afterAsync` / `rollback` are rejected at install time), only install / drop / list (no `start` / `stop` pause-resume), and no property-change-delta params (`$assignedNodeProperties` / `$removedLabels`). `databaseName` is accepted but ignored since Mesh is single-db today. Trigger registry replication works in both Raft and routing clusters, but **trigger-induced writes are local-only on the firing peer** in cluster modes — they don't replicate back through Raft / 2PC. Audit-style triggers writing to a single peer's local store work; triggers expecting cluster-wide writes don't. Closing this last gap needs an explicit "this commit is from a trigger, suppress further trigger firing" flag plumbed through `commit_buffered_commands` so trigger writes can recursively go through the cluster commit path without infinite-firing.
