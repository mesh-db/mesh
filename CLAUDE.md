...
- **Trigger procedures** — behind `apoc-trigger`: `apoc.trigger.install(databaseName, name, statement, selector, config)`, `apoc.trigger.drop(databaseName, name)`, `apoc.trigger.list()`. After a successful commit, registered triggers run their Cypher body with the tx diff exposed as `$createdNodes` / `$createdRelationships` / `$deletedNodes` / `$deletedRelationships` params. Triggers persist in a dedicated `trigger_meta` RocksDB CF and survive restart. A thread-local suppression guard prevents infinite self-triggering when a trigger's own writes would otherwise fire it again.
- **Deferred — trigger scope gaps.** The V1 cut only ships the `after` phase (`before` / `afterAsync` / `rollback` are rejected at install time), only install / drop / list (no `start` / `stop` pause-resume), and no property-change-delta params (`$assignedNodeProperties` / `$removedLabels`). `databaseName` is accepted but ignored since Mesh is single-db today. Trigger firing is **local-node only** — Raft and Routing commit paths skip trigger evaluation entirely, so clustered deployments need manual per-peer registration and the failure semantics of a trigger's writes under cluster partition are undefined. Adding cluster replication is the biggest outstanding piece: it needs a decision on leader-only vs every-peer firing, and on whether the trigger registry rides the same Raft log that metadata DDL does today.
