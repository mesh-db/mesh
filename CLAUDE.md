...
- **Not yet implemented:** edge-scope `PointIndexSeek` planner rewrite (DDL + storage are live); APOC procedure library.

### Query Execution (`meshdb-executor`)
- Volcano/iterator (pull-based) model — each operator implements `next() -> Option<Row>`.
- Operators: Scan, IndexSeek (composite-capable — tuple prefix seeks on the index CF), EdgeSeek (relationship-scope analogue for unbound-endpoints patterns), PointIndexSeek (Z-order bbox range + SRID-aware enclosing-bbox for distance queries), Expand, Filter, Project, Aggregate, Sort, Limit, CreateNode, CreateEdge, Delete.
