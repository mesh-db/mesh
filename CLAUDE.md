...
- **Not yet implemented:** `apoc.path.*` expansion, `apoc.cypher.run*` (needs runtime Cypher evaluation), `apoc.load.*` / `apoc.export.*` (file I/O and format parsers), `apoc.trigger.*` (DB-level event system).
...
The foundational phases are all shipping — single-node graph store, Cypher parser + executor, gRPC cluster RPCs, Raft-backed cluster membership, and a Bolt-speaking server. 2PC across partitions is hardened end-to-end: durable coordinator + participant logs, per-phase timeouts, idempotent PREPARE retries, and cross-peer decision recovery via `ResolveTransaction` so coordinator and participant crashes recover independently without operator intervention. The planner-side index work is caught up: composite property indexes (DDL + storage tuple encoding + prefix-matched seeks), edge IndexSeek for unbound-endpoint patterns (single-node, in-tx overlay, and cluster-routing scatter-gather), point / spatial indexes (Z-order cells + DDL + `withinbbox` and distance-radius planner rewrites for both node and relationship scope), and Neo4j 5 quantifier shorthand all shipped. APOC compatibility is now a fully-wired extension surface — nine scalar namespaces, the `apoc.agg.*` aggregates, write + read procedures for `apoc.create.*` / `apoc.refactor.setType` / `apoc.meta.schema`, and `apoc.periodic.iterate` running on top of the `CALL { } IN TRANSACTIONS` infrastructure (sequential batch dispatch only — a parallel worker pool is sketched but deferred until a workload calls for it). What remains (`apoc.path.*`, `apoc.cypher.run*`, `apoc.load.*` / `apoc.export.*`, `apoc.trigger.*`) each depends on new infrastructure (runtime Cypher evaluation, file I/O, event hooks) rather than more plumbing of the same pattern.
