...
APOC-compatible scalars live in the standalone `meshdb-apoc` crate; write / introspection procedures live beside the `db.*` built-ins in `meshdb-executor::procedures`. Each namespace is gated by a Cargo feature so default builds pay nothing; `--features apoc` on `meshdb-server` turns the full shipped surface on. Shipped today:
