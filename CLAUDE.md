...
- **Batched-write procedures** — `apoc.periodic.iterate(iterateQuery, actionQuery, config)` ships in-tree (no Cargo feature) as a planner-level rewrite into a dedicated `LogicalPlan::ApocPeriodicIterate` node that the server-side dispatcher unfolds into per-batch commits. Honors `batchSize`, `params`, `retries`, `failedParams`, and `iterateList`; emits the standard 13-column Neo4j result row (`batches`, `total`, `committedOperations`, `failedOperations`, `failedBatches`, `errorMessages`, `failedParams`, `batch`, `operations`, `updateStatistics`, `retries`, `timeTaken`, `wasTerminated`). Same explicit-tx rejection as `CALL { } IN TRANSACTIONS` — both forms commit per batch and conflict with an enclosing `BEGIN`/`COMMIT`.
- **Deferred — parallel-action runtime for `apoc.periodic.iterate`.** The procedure currently dispatches batches strictly sequentially. Neo4j's `parallel: true` / `concurrency: N` config keys are accepted as forward-compat no-ops. A real implementation needs a worker pool (tokio semaphore + `try_join_all` over chunks), `Arc<Mutex<...>>` around the stats / failedParams / updateStatistics accumulators, and care around ordering guarantees + concurrent commit failure semantics under Raft / 2PC contention. Sketched as ~100 lines + careful tests. Worth doing once a real workload needs the throughput; not before.
