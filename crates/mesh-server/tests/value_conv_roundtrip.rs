...
fn datetime_roundtrip_preserves_epoch_millis() {
    ...
    let input = Property::DateTime { nanos: 1_735_689_600_000, tz_offset_secs: Some(0), tz_name: None };
...
fn datetime_roundtrip_with_sub_second_fraction() {
    ...
    let input = Property::DateTime { nanos: 1_735_689_600_123, tz_offset_secs: Some(0), tz_name: None };
    let out = roundtrip_property(input.clone());
    assert_eq!(out, input);
}
...
fn datetime_roundtrip_pre_epoch() {
    ...
    // by one whole second.
    let input = Property::DateTime { nanos: -315_619_200_000, tz_offset_secs: Some(0), tz_name: None }; // 1960-01-01
    let out = roundtrip_property(input.clone());
    assert_eq!(out, input);
}
