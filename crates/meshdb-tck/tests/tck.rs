...
impl MeshWorld {
    ...
    fn run_cypher(&mut self, query: &str) {
        ...
        if let meshdb_cypher::Statement::Explain(inner) = &stmt {
            ...
            self.results = meshdb_executor::explain(&p);
            return;
        }
        ...
        match execute_with_reader_and_procs(
            &p,
            &self.store as &dyn meshdb_executor::GraphReader,
            &self.store as &dyn meshdb_executor::GraphWriter,
            &self.params,
            &self.procedures,
        ) {
...
fn given_named_graph(world: &mut MeshWorld, name: String) {
    // `tests/tck.rs` runs with `crates/meshdb-tck/` as CWD so the
...
fn format_iso_duration(d: &meshdb_core::Duration) -> String {
    let months = d.months;
    let days = d.days;
